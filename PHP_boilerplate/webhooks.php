...
//payment has failed, notfiy customer of problem
if ($event->type == 'invoice.payment_failed') {
	 $billingClass->requestBilling($customer);
}

//user did not update billing, account is no longer in subscription
if ($event->type == 'customer.subscription.deleted') {
 
   $billingClass->revokeUser($customer);	
}
//payment was successful, update subscription for another period
if ($event->type == 'customer.subscription.created' || $event->type == 'customer.subscription.updated') {
	 $plan = $event->data->object->plan->name;
 
	$billingClass->renewSubscription($customer);
	$billingClass->activateCustomer($customer, $plan);
...
/*function email_invoice_receipt($invoice) {
  ...
  $headers = 'From: "Boilerplate" <support@test.com>';
